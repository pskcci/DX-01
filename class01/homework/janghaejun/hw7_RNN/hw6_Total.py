...
history_RNN = model.fit(train_x, train_y,validation_data = (val_x, val_y),epochs=50, batch_size=20)

with open('history_RNN','wb') as file_pi:
    pickle.dump(history_RNN.history, file_pi)
model.save('RNN_mnist.h5')

pred_y_RNN = model.predict(test_x)
...
model.compile(optimizer='adam',loss='mean_squared_error', metrics=['accuracy'])
history_GRU = model.fit(train_x, train_y,validation_data = (val_x, val_y),epochs=70, batch_size=30)

with open('history_GRU','wb') as file_pi:
    pickle.dump(history_GRU.history, file_pi)
model.save('GRU_mnist.h5')

pred_y_GRU = model.predict(test_x)
...
pred_y_LSTM = model.predict(test_x)

plt.figure()
plt.plot(test_y, color = 'red', label = 'real SEC stock price')
plt.plot(pred_y_RNN, color = 'blue', label = 'predicted RNN SEC stock price')
plt.plot(pred_y_GRU, color = 'green', label = 'predicted GRU SEC stock price')
plt.plot(pred_y_LSTM, color = 'yellow', label = 'predicted LSTM SEC stock price')
